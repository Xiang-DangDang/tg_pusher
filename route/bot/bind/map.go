...
func (routeMap *Map) AddMessageRoute(path string, handler service.CommandHandler) {
	if path != "" {
		routeMap.messageMap[path] = NewRoute(path, handler, false, false)
	}
}

func (routeMap *Map) AddCommandRoute(path string, handler service.CommandHandler) {
	if path != "" {
		routeMap.commandMap[path] = NewRoute(path, handler, true, false)
	}
}

func (routeMap *Map) AddRegularExpression(path string, handler service.CommandHandler) {
	if path != "" {
